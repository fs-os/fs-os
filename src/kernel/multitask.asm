...
; Creates a new task named "name", and with the entry point "entry". Returns the ptr
; to the new task.
...
mt_newtask:
    ...
    ; First, move the current task's address to the new task's "prev" pointer, then
    ; move the current task's "next" pointer to the new task's "next" pointer,
    ...
    mov     [eax + ctx_t.stack], edx    ; Address of stack we just allocated. Stored
                                        ; so we can free it once the task ends.
...
; Frees the stack and ends the task passed as parameter. The task should not be the
...
mt_endtask:
    ...
    ; First, remove the Ctx* from the linked list. Make the previous item's "next"
    ; point to the "next" pointer of the task we want to free, and then make the
    ; next item's "prev" point to the "prev" pointer of the task we want to free:
    ;
    ...
    ; Free the stack we allocated for the task, and then free the Ctx struct as well

    push    eax         ; Preserve eax and push it to use it as arg for the 2nd free
                        ; call (first free the stack, then free the Ctx struct)

    push    dword [eax + ctx_t.stack]   ; Address of the allocated stack
    call    free                        ; Free the task's stack
    add     esp, 4                      ; Remove dword we just pushed

    call    free                        ; Free the Ctx struct itself, function arg
                                        ; we pushed before freeing the stack.
